<template>
  <qpage padding>
    <q-expansion-item
      expand-separator
      label="Applications and Toolkits"
      hide-expand-icon
      default-opened
    ...
    >
      <div class="q-pa-md q-gutter-md">
        <q-breadcrumbs>
          ...
          <q-breadcrumbs-el :label="connection" />
          <q-breadcrumbs-el label="Apps & Toolkits" />
        </q-breadcrumbs>
...
export default {
  setup() {
    ...
    function loadToolkits() {
      api
        .get(`api/getToolkits/${connection.value}`)
        .then((response) => {
          console.log(response.data.data.processAppsList);
          loading_toolkits.value = false;
          var data = response.data.data.processAppsList;
          toolkits.value.splice(0, toolkits.value.length, ...data);
        })
        .catch((e) => {
          console.log(e);
          $q.notify("Error loading toolkits");
          loading_toolkits.value = false;
        });
    }
    function loadApps() {
      api
        .get(`api/getApps/${connection.value}`)
        .then((response) => {
          console.log(response.data.data.processAppsList);
